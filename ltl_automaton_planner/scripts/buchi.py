...
def DuoBA_from_ltls(hard_spec, soft_spec):
    ...
    for f_duonode in DuoBA.nodes():
        for t_duonode in DuoBA.nodes():
            ...
            hardguard = DuoBA.graph['hard'][f_h_node][t_h_node]['guard']
            softguard = DuoBA.graph['soft'][f_s_node][t_s_node]['guard']
...
def check_label_for_buchi_edge(buchi, label, f_buchi_node, t_buchi_node):
    ...
    if buchi_type == 'hard_buchi':
        truth = buchi[f_buchi_node][t_buchi_node]['guard'].check(label)
        dist = 0
    if buchi_type == 'soft_buchi':
        truth = True
        dist = buchi[f_buchi_node][t_buchi_node]['guard'].distance(label)
    if buchi_type == 'safe_buchi':
        truth = buchi[f_buchi_node][t_buchi_node]['hardguard'].check(label)
        if truth:
            dist = buchi[f_buchi_node][t_buchi_node]['softguard'].distance(label)
        else:
            dist = 1000
    return truth, dist
